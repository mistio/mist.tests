...
def test_002_add_key_with_no_id_and_no_priv(pretty_print, mist_core,
    ...
    response = mist_core.add_key(id='', private='',
...
def test_003_add_key_with_no_private(pretty_print, cache, mist_core,
    ...
    response = mist_core.add_key(id=cache.get('keys_tests/key_id', ''),
                                 private='',
                                 api_token=valid_api_token).put()
    assert_response_bad_request(response)
    print "Success!!!"
...
def test_005_get_private_key_with_wrong_id(pretty_print, cache, mist_core,
                                           valid_api_token):
    response = mist_core.get_private_key('bla', api_token=valid_api_token).get()
...
def test_009_test_generate_keypair(pretty_print, mist_core, valid_api_token):
    response = mist_core.generate_keypair(api_token=valid_api_token).post()
    assert_response_ok(response)
    print "Success!!!"
...
def test_012_delete_multiple_keys_with_wrong_api_token(pretty_print,
                                                       ...
                                                       valid_api_token):
    response = mist_core.delete_keys(key_ids=['bla', 'bla2'],
                                     api_token=valid_api_token[:-2]).delete()
...
class TestSimpleUserKeyCycle:
    ...
    def test_add_key(self, pretty_print, cache, mist_core, valid_api_token,
        ...
        assert_response_ok(response)
        keys_list = json.loads(response.content)
        cache.set('keys_tests/simple_key_id', get_random_key_id(keys_list))
        ...
        response = mist_core.list_keys(api_token=valid_api_token).get()
        assert_response_ok(response)
        script = get_keys_with_id(cache.get('keys_tests/simple_key_id', ''),
    ...
    def test_edit_key(self, pretty_print, cache, mist_core, valid_api_token):
        ...
        assert_list_not_empty(script,
                              "Key was added through the api but is not "
                              "visible in the list of keys")
        cache.set('keys_tests/simple_key_id', new_key_id)
        print "Success!!!"
    ...
    def test_add_second_key_and_set_default(self, pretty_print, cache,
        ...
        response = mist_core.add_key(
            id=cache.get('keys_tests/other_key_id', ''),
            private=private_key,
            api_token=valid_api_token).put()
        assert_response_ok(response)
        response = mist_core.list_keys(api_token=valid_api_token).get()
    ...
    def test_delete_key(self, pretty_print, cache, mist_core, valid_api_token):
    ...
    def test_delete_multiple_keys(self, pretty_print, mist_core,
        ...
        for i in range(3):
            ...
            assert_response_ok(response)
            new_key_id = get_random_key_id(json.loads(response.content))
            response = mist_core.add_key(id=new_key_id, private=private_key,
                                         api_token=valid_api_token).put()
            assert_response_ok(response)
            response = mist_core.list_keys(api_token=valid_api_token).get()
            assert_response_ok(response)
