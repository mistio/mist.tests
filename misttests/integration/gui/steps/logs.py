...
def check_log_entry_dashboard_page(context, log_type, action, time_entry, page, timeout):
    if page in ['dashboard']:
        ...
        mist_header = mist_app_shadow.find_element(By.TAG_NAME, 'app-header-layout')
        iron_pages = mist_header.find_element(By.CSS_SELECTOR, '#iron-pages')
        pg_dashboard = iron_pages.find_element(By.TAG_NAME, 'page-dashboard')
        ...
        logs_list = container_shadow.find_element(By.ID, page + 'Logs')
    ...
    while time() < _timeout:
        log_elements = items.find_elements(By.TAG_NAME, 'tr')
