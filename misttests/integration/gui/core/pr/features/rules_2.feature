...
Feature: Rules
  ...
  Scenario: Insert rule regarding metric from rules page. If triggered, alert
    ...
    And I select the "load.shortterm" target when adding new rule in the "rules" page
    And I select the "<" operator when adding new rule in the "rules" page
    And I type "3" in the threshold when adding new rule in the "rules" page
    ...
    And I wait for 2 seconds
    Then rule "if load < 3 for every value within 1 minutes then alert team Owners" should be present in the "machine" page
    And I should receive an email at the address "EMAIL" which contains subject terms: "[Mist] *** WARNING *** machine `rules-test-machine-random`: Load" within 180 seconds
  ...
  Scenario: Delete a rule from rules page and verify it is not visible in single machine page
    ...
    And I search for "rules-test-machine-random"
    And I click the "rules-test-machine-random" "machine"
    Then rule "if load < 3 for every value within 1 minutes then alert team Owners" should be absent in the "machine" page
