...
def check_plan_overusage_message(context, text):
    plan_overusage = context.browser.find_element(By.TAG_NAME, 'plan-overusage')
...
def check_current_plan_message(context, text):
    plans = context.browser.find_elements(By.TAG_NAME, 'plan-item')
...
def upgrate_to_small_plan(context):
    plans = context.browser.find_elements(By.TAG_NAME, 'plan-item')
    small_plan = plans[1].find_element(By.TAG_NAME, 'paper-button')
...
def set_card_details(context):
    ...
    dialog = form.find_element(By.TAG_NAME, 'paper-dialog-scrollable')
