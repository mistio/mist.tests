...
def test_create_record_no_api_token(pretty_print, cache, mist_core):
    ...
    name = 'blog.' + domain
    response = mist_core.create_record(api_token='', cloud_id=cache.get('gce_cloud_id', ''),
...
def test_create_record_wrong_api_token(pretty_print, cache, mist_core):
    domain = cache.get('domain', '')
    name = 'blog.' + domain
    response = mist_core.create_record(api_token='dummy', cloud_id=cache.get('gce_cloud_id', ''),
                                       zone_id=cache.get('zone_id', ''),
                                       name=name, type='A',
                                       data="1.2.3.4", ttl=3600).post()
    assert_response_unauthorized(response)
