...
def test_list_schedules(pretty_print, mist_core, new_owner_api_token):
...
# def test_add_schedule_missing_parameter(pretty_print, mist_core, new_owner_api_token):
#     response = mist_core.add_schedule(api_token=new_owner_api_token, name='dummy').post()
...
def test_delete_schedule_wrong_schedule_id(pretty_print, mist_core, new_owner_api_token):
    response = mist_core.delete_schedule(api_token=new_owner_api_token, schedule_id='dummy').delete()
...
def test_edit_schedule_wrong_schedule_id(pretty_print, mist_core, new_owner_api_token):
    response = mist_core.edit_schedule(api_token=new_owner_api_token, schedule_id='dummy').patch()
    assert_response_not_found(response)
    print("Success!!!")
...
def test_show_schedule_wrong_schedule_id(pretty_print, mist_core, new_owner_api_token):
    response = mist_core.show_schedule(api_token=new_owner_api_token, schedule_id='').get()
    assert_response_not_found(response)
    print("Success!!!")
...
class TestSchedulesFunctionality:
    ...
    def test_create_resources(self, pretty_print, mist_core, new_owner_api_token, cache):
        if config.LOCAL:
        ...
        response = mist_core.list_images(cloud_id=cache.get('docker_id', ''), api_token=new_owner_api_token).get()
        assert_response_ok(response)
        ...
        for i in range(1,4):
            ...
            response = mist_core.create_machine(cloud_id=cache.get('docker_id', ''), api_token=new_owner_api_token,
        ...
        for machine in response.json():
            ...
            if cache.get('machine_2_name','') in machine['name']:
                ...
                assert machine['state'] == 'running',\
                    "Machine's state is not running in the beginning of the tests"
                # response = mist_core.set_machine_tags(api_token=new_owner_api_token,
    ...
    def test_add_one_off_schedule_missing_schedule_entry(self, pretty_print, mist_core, new_owner_api_token, cache):
        machines_uuids = []
        machines_uuids.append(cache.get('machine_1_id', ''))
        selectors = [{"type": "machines", "ids": machines_uuids}]
        response = mist_core.add_schedule(api_token=new_owner_api_token,
                                          name='TestSchedule1',
                                          action='stop',
    ...
    def test_add_interval_schedule_run_immediately_ok(self, pretty_print, mist_core, new_owner_api_token, cache):
        response = mist_core.list_machines(cloud_id=cache.get('docker_id', ''), api_token=new_owner_api_token).get()
        assert_response_ok(response)
        machines_uuids = []
        machines_uuids.append(cache.get('machine_1_id', ''))
        selectors = [{"type":"machines","ids":machines_uuids}]
        response = mist_core.add_schedule(api_token=new_owner_api_token, name='TestSchedule1',
    ...
    # def test_add_interval_schedule_tags_ok(self, pretty_print, mist_core, new_owner_api_token):
    #     selectors = [{"type":"tags","include":{'schedule_test': ''}}]
    #     response = mist_core.add_schedule(api_token=new_owner_api_token, name='TestSchedule2',
    #                                       action='stop', schedule_type='interval',
    #                                       selectors=selectors,
    #                                       run_immediately=True,
    ...
    def test_edit_crontab_schedule_ok(self, pretty_print, mist_core,
                                      cache, new_owner_api_token):
        response = mist_core.edit_schedule(api_token=new_owner_api_token,
...
# TODO: below does not apply to any machines yet
    def test_add_one_off_schedule_tags_ok(self, pretty_print, mist_core, new_owner_api_token, cache):
        date_now = datetime.datetime.now().replace(microsecond=0)
        scheduled_date = date_now + datetime.timedelta(seconds=10)
        selectors = [{"type": "tags", "include": {'schedule_test': ''}}]
        response = mist_core.add_schedule(api_token=new_owner_api_token, name='TestSchedule4',
    ...
    def test_delete_schedule_ok(self, pretty_print, mist_core, new_owner_api_token, cache):
        response = mist_core.delete_schedule(api_token=new_owner_api_token,
    ...
    def test_total_run_counts_disabled_schedule(self, pretty_print, mist_core, new_owner_api_token, cache):
        response = mist_core.show_schedule(api_token=new_owner_api_token,
    ...
    def test_check_schedules(self, pretty_print, mist_core, new_owner_api_token, cache, schedules_cleanup):
