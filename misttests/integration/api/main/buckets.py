...
def test_list_buckets_no_api_token(mist_core):
    response = mist_core.list_buckets().get()
    assert_response_unauthorized(response)
    print("Success!!!")


def test_list_buckets_wrong_api_token(mist_core):
    response = mist_core.list_buckets(api_token='dummy').get()
    assert_response_unauthorized(response)
    print("Success!!!")
...
def test_list_bucket_content_wrong_api_token(mist_core):
    response = mist_core.list_bucket_content(api_token='dummy',
                                             bucket_id='dummy').get()
    assert_response_unauthorized(response)
    print("Success!!!")


def test_list_bucket_content_wrong_bucket_id(mist_core, owner_api_token):
    response = mist_core.list_bucket_content(api_token=owner_api_token,
                                             bucket_id='dummy').get()
...
class TestBucketsFunctionality:
    ...
    def test_list_buckets(self, pretty_print, mist_core, cache,
                          owner_api_token):
